...
class _Test extends Model
{
    ...
    public static function set($test_id, $column, $new_value)
    {
        $db = new Database;
        try {
            $sql = 'UPDATE tests SET  ' .$column. ' = :new_value WHERE (tests.TEST_ID = :test_id) LIMIT 1';
            $query = $db->prepare($sql);
...
/**
 ...
 /**
    ...
    /**
     * Soragy cozup bilyami ? true : false
     *
     ...
     *
     */
    public static function canSolve($test_id, $user_id)
    {
        $db = new Database;
     ...
     *
     */
    public static function solvingFinish($test_id, $user_id)
    {
        $db = new Database;
        try{
            $sql = 'UPDATE result SET END_TIME=NOW() WHERE (TEST_ID = ? AND USER_ID=?)';
            $query = $db->prepare($sql);
            $query->execute([$test_id,$user_id]);
            return true;
        } catch (Exception $e) {
            echo $e;
        }
    ...
    /**
     * Test baradaky maglumatlary (test result ucin)
     *
     * @param int $test_id test ID
     * @author Agamyrat C
     *
    ...
    /**
     * Solving PostView ucin ve resulat ucin
     *
     * @param int $test_id test ID
     * @param int $user_id test ID
     * @author Agamyrat C
     *
     */
    public static function getSolvingScore($test_id, $user_id)
    {
        $db = new Database;
        try {
            $sql = "SELECT
                    ...
                    CASE
                            ...
                            WHEN ISNULL( result.END_TIME )* 60 THEN
                            tests.GIVEN_TIME * 60 ELSE TIMESTAMPDIFF( SECOND, result.START_TIME, result.END_TIME ) 
                        END TIME_SOLVING 
                    FROM
                        result
                        LEFT JOIN tests ON tests.TEST_ID = result.TEST_ID
            ...
            $result = $query->setFetchMode(PDO::FETCH_ASSOC);
            $fetchedDatas = $query->fetch();
            //print_r($fetchedDatas);die;
            $returnDatas["true_answers"] = $fetchedDatas['TRUE_ANSWER_COUNT'];
            $returnDatas["false_answers"] = $fetchedDatas['FALSE_ANSWER_COUNT'];
            $returnDatas["not_solved"] = $fetchedDatas['NOT_SOLVED'];
            $returnDatas["score"] = $fetchedDatas['SCORE'];
            $returnDatas["solved_time"] = $fetchedDatas['TIME_SOLVING'];
            return $returnDatas;
        } catch (Exception $e) {
            var_dump($e);
        }
