<?php return 
'<!DOCTYPE html>
    <html lang="en">
    <head>
        <meta charset="UTF-8" />
        <meta name="viewport" content="width=device-width, initial-scale=1.0" />
        <link rel="stylesheet" href="css/verification.css" />
        <link
        href="https://fonts.googleapis.com/css2?family=Poppins:wght@100;200;300;400;500;700&display=swap"
        rel="stylesheet"
        />
        <title>Verify Email</title>
    </head>
    
    <body style="background-color: #f5f5f5;">
        <div
        class="content"
        style="
            background-color: #fff;
            max-width: 600px;
            width: 90%;
            margin: 2em auto;
        "
        >
        <div style="text-align: center;">
            <img src="" alt="" style="width: 90%; max-width: 400px;" />
        </div>
        <h2 style="text-align: center;">Lorem, ipsum dolor.</h2>
        <p style="text-align: center;">
            Bu Email TestSpace test platformasy tarapyndan iberildi. Test
            platformasynda acan hasabynyzy aktiwlesdirmek ucin asakdaky duwma
            basmagynyzy hayys edyaris
        <div style="font-size: 14px;line-height: 2; display: inline-block;margin:10px auto;color:#fff;border-radius: 5px; padding:10px 10px; background-color:#fc7318;"><strong><a href="'.$link.'">AKTIW ET</a></strong></div>
        <div>Eger basyp bolmasa şu salgyny göçürip browseriňize ýazyp görüň!</div>
        <div>'.$link.'</div>
        </p>
        </div>
    </body>
    </html>';