<?php 
	class View {
		...
		public function renderCards($datas=null){
			if(!empty($datas['test_cards'])){
				$test_cards = $datas['test_cards'];
				foreach($test_cards as $test_card){
					require "views/".$this->pageName."/test_card.php";
				}
			}
			/*  for($i=0;$i<$count;$i++):
				require "views/".$this->pageName."/test_card.php";
