...
class Solving extends Model
{
   ...
   /**
    * cozulen question tazeden bosh edip iberilse kone jogaby pozmaly
    * 
    * @param int $solving_id pozmaly jogap id
    * @return bool true, false
    * @author Agamyrat C.
    * 
    */
   public static function deleteAnswer($solving_id)
   {
      $db = new Database;
      try {
         $sql = "DELETE FROM solving WHERE `SOLVING_ID`=:solving_id";
         $query = $db->prepare($sql);
         //var_dump($query);
         $query->execute(
            [
